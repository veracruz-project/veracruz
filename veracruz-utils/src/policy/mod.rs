...
pub mod error;
/// Expiry points-in-time for policies, certificates, and related material.
pub mod expiry;
/// Parsers for various types related to policies.
pub mod parsers;
/// Veracruz principals, and related meta-data.
pub mod principal;
...
pub struct Policy {
    ...
    /// Whether to enable the clock-related WASI system calls.
    enable_clock: bool,
...
impl Policy {
    ...
    /// well-formedness checks pass.  Sets all hash-related values to `None`,
    /// all principals, programs, and the standard stream table to `Vec::new()`.
    #[inline]
    pub fn new(
        veracruz_server_url: String,
        enclave_cert_expiry: Timepoint,
        ...
        let policy = Self {
            ...
            enclave_cert_expiry,
            ciphersuite,
            runtime_manager_hash_sgx: None,
            runtime_manager_hash_tz: None,
            runtime_manager_hash_nitro: None,
            proxy_attestation_server_url,
            debug: false,
            execution_strategy,
            std_streams_table: Vec::new(),
            enable_clock: false,
            policy_hash: None,
        };
    ...
    /// Sets the standard streams table associated with the policy to `streams`.
    #[inline]
    pub fn set_std_streams_table(&mut self, streams: Vec<StandardStream>) -> &mut Self {
        self.std_streams_table = streams;
        self
    }

    /// Sets the clock enablement status to `enable_clock`.
    #[inline]
    pub fn set_enable_clock(&mut self, enable_clock: bool) -> &mut Self {
        self.enable_clock = enable_clock;
        self
    }

    /// Sets the programs associated with the policy to `programs`.
    #[inline]
    pub fn set_programs(&mut self, programs: Vec<Program>) -> &mut Self {
        self.programs = programs;
        self
    }

    /// Sets the identities associated with the policy to `identities`.
    #[inline]
    pub fn set_identities(&mut self, identities: Vec<Identity<String>>) -> &mut Self {
        self.identities = identities;
        self
    }

    /// Sets the debug flag of the policy to `debug`.
    #[inline]
    ...
    /// Sets the SGX runtime manager hash to `hash`.
    #[inline]
    pub fn set_runtime_manager_hash_sgx(&mut self, hash: String) -> &mut Self {
        self.runtime_manager_hash_sgx = Some(hash);
        self
    }

    /// Sets the Nitro runtime manager hash to `hash`.
    #[inline]
    pub fn set_runtime_manager_hash_nitro(&mut self, hash: String) -> &mut Self {
        self.runtime_manager_hash_nitro = Some(hash);
        self
    }

    /// Sets the TrustZone runtime manager hash to `hash`.
    #[inline]
    pub fn set_runtime_manager_hash_tz(&mut self, hash: String) -> &mut Self {
        self.runtime_manager_hash_tz = Some(hash);
        self
    }
    ...
    pub fn from_json<A>(json: A) -> Result<Self, PolicyError>
    where
        A: AsRef<str>,
    {
        // parse json
    ...
    /// Returns the clock enablement status.
    #[inline]
    ...
    pub fn rights_table(&self) -> RightsTable {
    ...
    pub fn digest_table(&self) -> Result<HashMap<String, Vec<u8>>, PolicyError> {
        let mut table = HashMap::new();
        for program in &self.programs {
            let program_file_name = program.program_file_name();
    ...
    pub fn input_table(&self) -> Result<HashMap<String, Vec<PathBuf>>, PolicyError> {
        let mut table = HashMap::new();
        for program in &self.programs {
            let program_file_name = program.program_file_name();
            ...
            table.insert(
                program_file_name.to_string(),
                Self::required_inputs(&file_rights_map),
            );
        }
        Ok(table)
    }
    ...
    fn required_inputs(right_map: &HashMap<PathBuf, Rights>) -> Vec<PathBuf> {
