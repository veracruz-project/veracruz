...
/// An enumerated type describing messages passed between Mexico City
...
pub enum NitroRootEnclaveMessage {
    ...
    /// A request to set the mexico city hash that the Nitro Root Enclave
    ...
    /// A request (usually initiated by the Mexico City enclave) to start the
...
/// An enumerated type describing messages passed between to/from the Mexico
/// City enclave (These originate from the Untrusted Pass-through (Sinaloa)
/// These messages are inteded to be serialized using bincode before transport,
/// and deserialized using bincode after transport
#[derive(Serialize, Deserialize, Debug)]
pub enum MCMessage {
    ...
    /// NitroStatus - the Status
    Status(NitroStatus),
    /// A request to initialize the Mexico City enclave with the provided
    /// policy
    /// parameters:
    ...
    EnclaveName(String),
    /// A request for a PSA Attestation (proxy) token from the Mexico City
    /// enclave.
    /// Parameters:
