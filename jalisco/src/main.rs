...
use std::io::Write;
use veracruz_utils::JaliscoOpcode;
...
lazy_static! {
    ...
    // It should be the hash value of the current program (jalisco), and it
    ...
    static ref JALISCO_HASH: Vec<u8> = vec![
...
fn create() -> optee_utee::Result<()> {
    ...
    if status != 0 {
        trace_println!("jalisco::create psa_initial_attest_load_key failed to load device private key with code:{:}", status);
...
fn open_session(_params: &mut Parameters) -> optee_utee::Result<()> {
    trace_println!("jalisco:open_session");
    return Ok(());
}

#[ta_close_session]
fn close_session() {
    trace_println!("jalisco:close_session");
}

#[ta_destroy]
fn destroy() {
    trace_println!("jalisco:destroy");
}

#[ta_invoke_command]
fn invoke_command(cmd_id: u32, params: &mut Parameters) -> optee_utee::Result<()> {
    trace_println!("jalisco:invoke_comand");
    let cmd = JaliscoOpcode::from_u32(cmd_id).map_err(|_| ErrorKind::BadParameters)?;
    match cmd {
        JaliscoOpcode::GetFirmwareVersionLen => {
            trace_println!("jalisco::invoke_command GetFirmwareVersionLen");
            let mut values = unsafe {
                params.0.as_value().map_err(|err| {
                println!("jalisco::invoke_command JaliscoOpcode::GetFirmwareVersionLen failed to get params.0:{:?}", err);
                ErrorKind::Unknown
            })?
            };
        ...
        JaliscoOpcode::GetFirmwareVersion => {
            trace_println!("jalisco::invoke_command GetFirmwareVersion");
            let mut p0 = unsafe {
                params.0.as_memref().map_err(|err| {
                println!("jalisco::invoke_command JaliscoOpcode::GetFirmwareVersion failed to get params.0.as_memref:{:?}", err);
                ErrorKind::Unknown
            })?
            };
            let version = env!("CARGO_PKG_VERSION");
            p0.buffer().write(&version.as_bytes()).map_err(|err| {
                println!("jalisco::invoke_command JaliscoOpcode::GetFirmwareVersion failed to write to buffer:{:?}", err);
                ErrorKind::Unknown
            })?;
        }
        JaliscoOpcode::SetMexicoCityHashHack => {
            ...
            trace_println!("jalisco::invoke_command SetMexicoCityHashHack");
            let hash_value = unsafe {
                let mut memref = params.0.as_memref().map_err(|err| {
                    trace_println!("jalisco::invoke_command::SetMexicoCityHashHack failed to get params.0 as mem_ref:{:?}", err);
                    ErrorKind::BadParameters
                })?;
                memref.buffer().to_vec()
            };
            let mut mch_guard = MEXICO_CITY_HASH.lock().map_err(|err| {
                trace_println!("jalisco::invoke_command::SetMexicoCityHashHack failed to obtain lock on MEXICO_CITY_HASH:{:?}", err);
                ErrorKind::TargetDead
            })?;
            *mch_guard = Some(hash_value);
        }
        JaliscoOpcode::NativeAttestation => {
            trace_println!("jalisco::invoke_command NativeAttestation Opcode started");
            let mut values = unsafe {
                params.0.as_value().map_err(|err| {
                    trace_println!(
                        "jalisco::NativeAttestation failed to extract values from parameters:{:?}",
                        err
                    );
                    ErrorKind::Unknown
                })?
            ...
            let device_id: i32 = values.a().try_into().map_err(|err| {
                trace_println!(
                    "jalisco::invoke_command NativeAttestation failed to convert a into i32:{:?}",
                    err
                );
                ErrorKind::Unknown
            })?;
            ...
            let challenge = unsafe {
                let mut memref = params.1.as_memref().map_err(|err| {
                    trace_println!("jalisco::invoke_command::NativeAttestation failed to get params.1 as mem_ref:{:?}", err);
                    ErrorKind::BadParameters
                })?;
                memref.buffer().to_vec()
            };
            ...
            let mut token_buf = unsafe {
                params.2.as_memref().map_err(|err| {
                    trace_println!("jalisco::NativeAttestation failed to extract token_buf from parameters:{:?}", err);
                    ErrorKind::Unknown
                })?
            };
            ...
            let mut pubkey_buf = unsafe {
                params.3.as_memref().map_err(|err| {
                    trace_println!("jalisco::NativeAttestation failed to extrac public key buffer from parameters:{:?}", err);
                    ErrorKind::Unknown
                })?
            };
            trace_println!("jalisco::invoke_command calling native_attestation function");
            let token = native_attestation(device_id, &challenge).map_err(|err| {
                trace_println!(
                    "jalisco::invoke_command call to native_attestation failed:{:?}",
                    err
                );
                ErrorKind::TargetDead
            })?;
            trace_println!("jalisco::invoke_command returned from native_attestation function");
            token_buf.buffer().write(&token).map_err(|err| {
                trace_println!(
                    "jalisco::NativeAttestation failed to place token in token_buf:{:?}",
                    err
                );
                ErrorKind::Unknown
            })?;
            trace_println!("jalisco::invoke_command setting token.len:{:}", token.len());
            values.set_b(token.len() as u32);

            let public_key = {
                let dpk_guard = DEVICE_PUBLIC_KEY.lock().map_err(|err| {
                    trace_println!(
                        "jalisco::native_attestation failed to obtain lock on PUBLIC_KEY_HASH:{:}",
                        err
                    );
                    ErrorKind::Unknown
                })?;
            ...
            pubkey_buf.buffer().write(&public_key).map_err(|err| {
                trace_println!(
                    "jalisco::NativeAttestation failed to place public key in pubkey_buf:{:?}",
                    err
                );
                ErrorKind::Unknown
            })?;
        ...
        JaliscoOpcode::ProxyAttestation => {
            // p0 - challenge input
            // p1 - enclave_cert input / Jalisco Pubkey Output
            // p2 - token output
            // p3 - a: device_id output b:none
            trace_println!("jalisco::invoke_command ProxyAttestation Opcode started");
            let challenge = unsafe {
                trace_println!(
                    "jalisco::invoke_command ProxyAttestation params.0.param_type:{:?}",
                    params.0.param_type
                );
                trace_println!("jalisco::invoke_command ProxyAttestation params.0.raw:");
                ...
                let mut memref = params.0.as_memref().map_err(|err| {
                    trace_println!("jalisco::invoke_command ProxyAttestation failed to get params.0 as memref:{:?}", err);
                    ErrorKind::BadParameters
                })?;
                memref.buffer().to_vec()
            };
            trace_println!(
                "jalisco::invoke_command ProxyAttestation challenge:{:?}",
                challenge
            );
            trace_println!(
                "jalisco::invoke_command ProxyAttestation challenge.len: {:?}",
                challenge.len()
            );
            let mut cert_pubkey_buffer = unsafe {
                let memref = params.1.as_memref().map_err(|err| {
                    trace_println!(
                        "jalisco::invoke_command failed to get memref from params.1:{:?}",
                        err
                    );
                    ErrorKind::TargetDead
                })?;
            ...
            let token = proxy_attestation(&challenge, &cert).map_err(|err| {
                trace_println!("jalisco::invoke_command proxy_attestation failed:{:?}", err);
                ErrorKind::TargetDead
            })?;
            let mut param3_value = unsafe {
                params.3.as_value().map_err(|err| {
                    trace_println!(
                    "jalisco::invoke_command ProxyAttestation failed to get params.3 as_value:{:?}",
                    err
                );
                    ErrorKind::TargetDead
                })?
            ...
            let device_id = {
                let di_guard = DEVICE_ID.lock().map_err(|err| {
                                       trace_println!("jalisco::invoke_command ProxyAttestation failed to get lock on DEVICE_ID:{:?}", err);
                                       ErrorKind::TargetDead
                                   })?;
                di_guard.unwrap()
            };
            ...
            let device_public_key = {
                let dpk_guard = DEVICE_PUBLIC_KEY.lock().map_err(|err| {
                    trace_println!("jalisco::invoke_command ProxyAttestation failed to get lock on DEVICE_PUBLIC_KEY:{:?}", err);
                    ErrorKind::TargetDead
                })?;
                (*dpk_guard).clone().unwrap()
            };
            trace_println!(
                "jalisco::invoke_command ProxyAttestation has device_public_key:{:?}",
                device_public_key
            );
            param3_value.set_a(device_id.try_into().map_err(|err| {
                trace_println!("jalisco::invoke_command ProxyAttestation failed to convert device_id into u32:{:?}", err);
                ErrorKind::Unknown
            })?);

            token_buf.buffer().write(&token).map_err(|err| {
                trace_println!(
                    "jalisco::ProxyAttestation failed to place token in token_buf:{:?}",
                    err
                );
                ErrorKind::Unknown
            })?;
            ...
            cert_pubkey_buffer.buffer().write(&device_public_key).map_err(|err| {
                trace_println!("jalisco::invoke_command::ProxyAttestation faeild to place pubkey in cert_pubkey_buffer:{:?}", err);
                ErrorKind::Unknown
            })?;
            unsafe {
    ...
    trace_println!("jalisco::invoke_command done");
    return Ok(());
}
...
fn native_attestation(device_id: i32, challenge: &Vec<u8>) -> Result<Vec<u8>, String> {
    ...
    if status != 0 {
        return Err(format!(
            "jalisco::create psa_initial_attest_load key failed with code:{:}",
            status
        ))?;
    }
    ...
    // Save the device_id
    {
        let mut di_guard = DEVICE_ID.lock().map_err(|err| {
            format!(
                "jalisco::native_attestation failed to obtain lock on DEVICE_ID:{:?}",
                err
            )
        })?;
    ...
    let device_public_key_hash: Vec<u8> = {
        ...
        let dpk = dpk_guard.clone().unwrap();
        trace_println!(
            "jalisco::native_attestation calculating hash of device public key:{:?}",
            dpk
        );
        let pubkey_hash = ring::digest::digest(&ring::digest::SHA256, dpk.as_ref());
        trace_println!(
            "jalisco::native_attestation calculated hash:{:?}",
            pubkey_hash
        );
    ...
    // if result != 0 {
    //     return Err(format!(
    //         "jalisco::native_attestation TEE_GetPropertyAsIdentity failed:{:}",
    //         result
    //     ));
    // }

    let mut jalisco_hash: [u8; 32] = [0; 32];
    jalisco_hash.clone_from_slice(&JALISCO_HASH[0..32]);
    ...
    let status = unsafe {
        psa_initial_attest_get_token(
            &jalisco_hash as *const u8,
            jalisco_hash.len() as u64,
    ...
    if status != 0 {
        return Err(format!(
            "jalisco::native_attestation psa_initial_attest_get_token failed with error code:{:}",
            status
        ));
    }
    unsafe { token_buffer.set_len(token_size as usize) };
    trace_println!(
        "jalisco::native_attestation token_buffer value:{:?}",
        token_buffer
    );
...
fn proxy_attestation(challenge: &Vec<u8>, enclave_cert: &Vec<u8>) -> Result<Vec<u8>, String> {
    let device_private_key = {
        let dpk_guard = DEVICE_PRIVATE_KEY.lock().map_err(|err| {
            format!(
                "jalisco::proxy_attestation failed to obtain lock on DEVICE_PRIVATE_KEY:{:?}",
                err
            )
        })?;
        match &*dpk_guard {
    ...
    trace_println!("jalisco::proxy_attestation started");
    let mexico_city_hash = {
        let mch_guard = MEXICO_CITY_HASH.lock().map_err(|err| {
            format!(
                "jalisco::proxy_attestation failed to obtain lock on MEXICO_CITY_HASH:{:?}",
                err
            )
        })?;
        match &*mch_guard {
            Some(hash) => hash.clone(),
            None => return Err(format!("jalisco::proxy_attestation MEXICO_CITY_HASH does not contain data and that's a problem")),
        }
    };
    ...
    if status != 0 {
        return Err(format!(
            "jalisco::proxy_attestation psa_initial_attest_get_token failed with error code:{:}",
            status
        ));
    }
    unsafe { token.set_len(token_len as usize) };
    trace_println!("jalisco::proxy_attestation token_buffer value:{:?}", token);
    Ok(token.clone())
}
...
const TA_DESCRIPTION: &[u8] = b"Jalisco TA for Veracruz\0";
const EXT_PROP_VALUE_1: &[u8] = b"Jalisco TA\0";
