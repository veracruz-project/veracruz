...
impl VeracruzClient {
        ...
        P2: AsRef<path::Path>,
    {
        ...
        let proxy_service_cert = {
            ...
            let certs =
                rustls::internal::pemfile::certs(&mut certs_pem.as_bytes()).map_err(|_| {
                    VeracruzClientError::X509ParserError(String::from(
        ...
        let client_cert = client_cert_raw
            .replace(
                "-----BEGIN CERTIFICATE-----",
                "-----BEGIN CERTIFICATE-----\n",
            )
    ...
    pub fn send_program(
        ...
        match parsed_response.get_status() {
            transport_protocol::ResponseStatus::SUCCESS => Ok(()),
            otherwise => Err(VeracruzClientError::ResponseError(
                "send_program",
                otherwise,
            )),
        }
    ...
    pub fn send_data(&mut self, file_name: &str, data: &[u8]) -> Result<(), VeracruzClientError> {
        self.check_policy_hash()?;
        self.check_runtime_hash()?;
        ...
        match parsed_response.get_status() {
            transport_protocol::ResponseStatus::SUCCESS => Ok(()),
            otherwise => Err(VeracruzClientError::ResponseError("send_data", otherwise)),
        }
    }
            ...
            Some(certs) => {
                ...
                match ues.get(&encoded_extension_id[..]) {
                    ...
                    Some(data) => {
                        ...
                        if let Err(err) =
                            self.compare_runtime_hash(extension_data.as_slice_less_safe())
                        {
    ...
    fn send(&mut self, data: &[u8]) -> Result<Vec<u8>, VeracruzClientError> {
        ...
        // NB: Intentionally sending no data to process, here.
        if let Some(outgoing_data) = self.process(Vec::new())? {
        ...
        loop {
            for outgoing_data in &outgoing_data_vec {
                if let Some((received_session_id, received_data_vec)) =
            ...
            if !incoming_data_vec.is_empty() {
                for incoming_data in &incoming_data_vec {
                    if let Some(outgoing_data) = self.process(incoming_data.to_vec())? {
                        outgoing_data_vec.push(outgoing_data);
                    }
                }
            ...
            if let Some(plaintext_data) = self.get_data()? {
    ...
    fn process(&mut self, input: Vec<u8>) -> Result<Option<Vec<u8>>, VeracruzClientError> {
        ...
        if !input.is_empty()
            && (!self.tls_session.is_handshaking() || self.tls_session.wants_read())
        {
            let mut slice = &input[..];
    ...
    fn get_data(&mut self) -> Result<Option<std::vec::Vec<u8>>, VeracruzClientError> {
        ...
        if read_received.is_ok() && !received_buffer.is_empty() {
    ...
    fn post_runtime_manager(
        ...
        if !body_items.is_empty() {
            ...
            for item in body_items.iter().skip(1) {
                return_vec.push(base64::decode(item)?);
            }
...
fn print_hex(data: &[u8]) -> String {
    ...
    for this_byte in data {
        ret_val.push_str(&format!("{:02x}", this_byte));
    }
