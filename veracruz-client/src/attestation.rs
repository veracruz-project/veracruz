...
use veracruz_utils::{platform::Platform, policy::Policy};
...
impl Attestation for AttestationPSA {
    ...
    fn attestation(
        ...
        let runtime_manager_hash = policy
            .runtime_manager_hash(target_platform)
            .map_err(|err| {
                println!(
                    "Did not find Runtime Manager hash for platform in policy:{:?}",
                    err
                );
                err
            })?;
...
impl AttestationPSA {
    fn attestation_flow(
        ...
        let complete_proxy_attestation_server_url =
            format!("http://{:}/VerifyPAT", proxy_attestation_server_url);
        let received_buffer =
            AttestationPSA::post_string(&complete_proxy_attestation_server_url, received_string)?;
        ...
        if *expected_enclave_hash != received_payload[47..79] {
            return Err(VeracruzClientError::MismatchError {
    ...
    fn post_veracruz_server(remote_url: &str, data: &[u8]) -> Result<String, VeracruzClientError> {
        let string_data = base64::encode(data);
        let dest_url = format!("http://{:}/veracruz_server", remote_url);
        AttestationPSA::post_string(&dest_url, string_data)
    }
