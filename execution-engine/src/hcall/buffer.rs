...
use veracruz_utils::policy::principal::{CapabilityTable, FileOperation, Principal};

/// Error type for mexico-city buffer.
#[derive(Clone, Debug, Error, Serialize, Deserialize)]
pub enum VFSError {
    //TODO: potential remove this
    #[error(display = "VFSError: File {} cannot be found.", _0)]
    FileNotFound(String),
    #[error(display = "VFSError: Principal or program {:?} cannot be found.", _0)]
    IndexNotFound(Principal),
    #[error(
        display = "VFSError: Client {:?} is disallowed to {:?}.",
        client_id,
        operation
    )]
    CapabilityDenial {
    ...
    #[error(
        display = "VFSError: File {:?} digest mismatches, model: {:?}, received: {:?}.",
        file_name,
        model,
        received
    )]
...
impl VFS {
    ...
    fn digest_check(&self, file_name: &str, new_data: &[u8]) -> Result<(), VFSError> {
        if let Some(digest) = self.digests.get(file_name) {
            ...
            if new_digest.len() != digest.len() {
                return Err(VFSError::FileDigestMismatch {
                    file_name: file_name.to_string(),
                    model: digest.to_vec(),
                    received: new_digest.to_vec(),
                });
            }
    ...
    pub fn count(&self, prefix: &str) -> Result<u64, VFSError> {
        Ok(self
            .fs
            .keys()
            .map(|k| if k.starts_with(prefix) { 1 } else { 0 })
            .sum())
    }
