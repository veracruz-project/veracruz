    ...
    loop {
        ...
        let (active_flag, output_data_option) = {
            ...
            let enclave = enclave_handler_locked
                .as_mut()
                .ok_or(VeracruzServerError::UninitializedEnclaveError)?;
            tls_data(session_id, buf[0..n].to_vec(), enclave)?
        };
...
pub fn new_tls_session(enclave: &mut Box<impl VeracruzServer + Send + Sync + ?Sized>) -> Result<u32, VeracruzServerError> {
...
pub fn tls_data(
    session_id: u32,
    input: Vec<u8>,
    enclave: &mut Box<impl VeracruzServer + Send + Sync + ?Sized>,
