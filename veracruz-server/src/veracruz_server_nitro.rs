...
use std::io::Read;
use veracruz_utils::{
    platform::nitro::{nitro_enclave::NitroEnclave, NitroStatus, RuntimeManagerMessage},
    policy::Policy,
};
...
impl Drop for VeracruzServerNitro {
    ...
    fn drop(&mut self) {
        if let Err(err) = self.close() {
            println!("VeracruzServerNitro::drop failed in call to self.close:{:?}, we will persevere, though.", err);
        }
    }
}
...
fn post_buffer(url: &str, buffer: &str) -> Result<String, VeracruzServerError> {
    let mut buffer_reader = stringreader::StringReader::new(buffer);

    let mut curl_request = Easy::new();
    curl_request
        .url(&url)
        .map_err(VeracruzServerError::CurlError)?;
    ...
    let mut received_header = String::new();
    {
            ...
            .write_function(|buf| {
                received_body.push_str(
                    std::str::from_utf8(buf)
                        .unwrap_or_else(|_e| panic!("Error converting data {:?} from UTF-8", buf)),
                );
                Ok(buf.len())
            })
            .map_err(VeracruzServerError::CurlError)?;
        ...
        transfer.perform().map_err(VeracruzServerError::CurlError)?;
    }
