...
> {
    ...
    let p_quote = unsafe { p_msg3_byte.add(quote_offset) as *mut sgx_quote_t };
    ...
    let bindgen_target_info_ref = unsafe {
        &*(&target_info as *const sgx_target_info_t as *const sgx_root_enclave_bind::_target_info_t)
    };
    let bindgen_collateral_report_ref = unsafe {
        &mut *(&mut collateral_report as *mut sgx_report_t as *mut sgx_root_enclave_bind::_report_t)
    };
...
impl VeracruzServerSGX {
    fn send_msg3(
        ...
        if parsed.has_cert_chain() {
            ...
            Ok((root_cert, enclave_cert))
        } else {
            Err(VeracruzServerError::MissingFieldError("cert_chain"))
        }
    }
}
...
pub extern "C" fn start_local_attest_ocall(
    ...
    let bindgen_msg1_ref = unsafe {
        &*(dh_msg1 as *const sgx_dh_msg1_t as *const sgx_root_enclave_bind::_sgx_dh_msg1_t)
    };
    let bindgen_msg2_ref = unsafe {
        &mut *(dh_msg2 as *mut sgx_dh_msg2_t as *mut sgx_root_enclave_bind::_sgx_dh_msg2_t)
    };
    ...
    match &*sgx_root_enclave {
        Some(sgx_root_enclave) => {
            ...
            let bindgen_msg3_ref = unsafe {
                &*(dh_msg3 as *const sgx_dh_msg3_t as *const sgx_root_enclave_bind::_sgx_dh_msg3_t)
            };
...
#[no_mangle]
pub extern "C" fn debug_and_error_output_ocall(message: *const c_char, code: u32) -> sgx_status_t {
