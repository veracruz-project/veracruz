...
impl WasmtimeRuntimeState {
    ...
    pub(crate) fn invoke_engine(&self, binary: Vec<u8>) -> Result<u32> {
        ...
        // TODO add all preopened_dir.
        let wasm_build = self.preopened_dir.iter().fold(Ok(wasm_build), |acc : Result<WasiCtxBuilder>, x| {
            let wasm_build = acc?;
            info!("preopened_dir: {:?}", x);
            // TODO we need to map to different space?
            let file = File::open(&x)?;
            Ok(wasm_build.preopened_dir(Dir::from_std_file(file), x)?)
        })?;
        //let path = Path::new(".");
